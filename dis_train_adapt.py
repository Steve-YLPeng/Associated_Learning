...
def main():
    ...
    if args.task == "text" or args.task == "classification":
        ...
        for epoch in range(args.epoch):
            ...
            with torch.no_grad():
                ...
                for threshold in test_threshold:
                    ...
                    torch.cuda.synchronize()
                    print(f'Test Epoch{epoch} threshold {threshold} Acc {acc}, AUC {AUC}, avg_entr {entr}, f1 {f1}')
                    print("ep%s_t%s_test_time %s"%(epoch, threshold ,time.process_time()-ep_test_start_time))
        ...
        print('valid_acc', numpy.array(model.history["valid_acc"]).shape)
        print('valid_AUC', numpy.array(model.history["valid_AUC"]).shape)
